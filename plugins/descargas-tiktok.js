...
conn.sendFile(m.chat, dataF.play, 'tiktok.mp4', `『✦』 ${mid.user}\n*${nickname}*\n${description ? '\n${mid.smsYT14}\n*${description}*' : ''}\n${wm}`.trim(), m) 
} catch (e1) {
try {
const tTiktok = await tiktokdlF(args[0])
conn.sendFile(m.chat, tTiktok.video, 'tiktok.mp4', `『✦』 ${mid.user}\n*${nickname}*\n${description ? '\n${mid.smsYT14}\n*${description}*' : ''}\n${wm}`.trim(), m) 
} catch (e2) {
try {
let p = await fg.tiktok(args[0]) 
conn.sendFile(m.chat, p.nowm, 'tiktok.mp4', `『✦』 ${mid.user}\n*${nickname}*\n${description ? '\n${mid.smsYT14}\n*${description}*' : ''}\n${wm}`.trim(), m)
} catch (e3) {
try { 
const { author: { nickname }, video, description } = await tiktokdl(args[0])
const url = video.no_watermark2 || video.no_watermark || 'https://tikcdn.net' + video.no_watermark_raw || video.no_watermark_hd
conn.sendFile(m.chat, url, 'tiktok.mp4', `『✦』 ${mid.user}\n*${nickname}*\n${description ? `\n${mid.smsYT14}\n*${description}*` : ''}\n${wm}`.trim(), m)
handler.limit = 2
} catch (e) {
